...
export const ArchiveTab = ({ beans }) => {
  ...
  return (
    <div>
      <div style={{ fontFamily: fonts.title, fontSize: 26, color: C.text, marginBottom: 4 }}>Archive</div>
      ...
      {finished.map(bean => (
        <div key={bean.id} style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              <div style={{ fontFamily: fonts.title, fontSize: 16, color: C.text }}>{bean.name}</div>
