...
export function gradeC5Numbers({ reply = '', userUnits = {} } = {}) {
  ...
  const counterfactualAlternative = /(?:^|;\s*)\s*(?:more|less|higher|lower)\s+(?:coffee|dose|water|heat|temperature|time|agitation|extraction)\b[^.!?]*\bwould\b[^.!?]*(?:\bbut\b|\band\s+(?:could|would|may|might)\s+(?:leave|keep|preserve)\b|\bwithout\s+(?:addressing|fixing|changing)\b)/i;
  ...
  const explanatoryTest = /(?:^|;\s*)\s*(?:this|that)\s+(?:is|would be)\s+(?:the\s+)?(?:cleanest|best|safest|simplest)\s+(?:first|next)?\s*(?:test|move|change|step)\s+for\s+(?:a\s+little\s+)?more\s+extraction\b/i;
  const predictedOutcome = /(?:^|;\s*)\s*(?:the\s+(?:finer|coarser)\s+grind|the\s+change|this(?:\s+change)?|that(?:\s+change)?)\s+(?:should|would|could)\s+(?:address|help|increase|decrease|improve|reduce|preserve|keep|add)\b/i;
...
export function gradeEvidenceScope({ reply = '', readWindow = null, evidence = {} } = {}) {
  ...
  if (/\bno\s+(?:(?:linked|separate|recorded)\s+)?(?:tasting|brew|recipe)(?:\s+note)?\s+(?:is\s+)?(?:attached|linked)\s+to\b|\bno\s+(?:tasting|brew|recipe)\s+for\s+that\s+(?:brew|cup)\b|\b(?:v60|kalita|aiden|brew|cup)\b[^?!]{0,128}\b(?:had|has|with)\b[^?!]{0,64}\bno\s+(?:(?:linked|recorded)\s+)?tasting(?:\s+note)?\b/i.test(value)) return [];
