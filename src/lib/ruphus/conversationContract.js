...
export function gradeC5Numbers({ reply = '', userUnits = {} } = {}) {
  ...
  const sized = /\b\d+(?:\.\d+)?\s*(?:g|grams?|ml|°?[CF]|steps?|clicks?|notches?|degrees?|seconds?|min(?:ute)?s?)\b|\b(?:one|two|three|a|an|another)\s+(?:(?:small|tiny|half|gentle|modest)\s+)?(?:(?:finer|coarser)\s+)?(?:grind\s+|dose\s+|ode\s+)?(?:step|gram|click|notch|degree|adjustment|increase|decrease|change)s?(?:\s+(?:finer|coarser))?\b|\b(?:a|an)\s+(?:gentle|modest|small|slight)\s+(?:extraction|strength|temperature|agitation)\s+(?:increase|decrease)\b|\b(?:a\s+)?half\s+(?:a\s+)?(?:step|click|notch|degree)s?\b|\b(?:a\s+(?:little|touch|bit)|slightly)\s+(?:more|less)\s+(?:extraction|agitation|time|heat|water|coffee|dose)\b|\bfrom\s+(?:[A-Za-z]+\s+)?\d+(?:\.\d+)?\s+(?:to|→)\s+\d+(?:\.\d+)?\b/i;
  ...
  const counterfactualAlternative = /(?:^|;\s*)\s*(?:more|less|higher|lower)\s+(?:coffee|dose|water|heat|temperature|time|agitation|extraction)\b[^.!?]*\bwould\b[^.!?]*(?:\bbut\b|\band\s+(?:could|would|may|might)\s+(?:leave|keep|preserve)\b|\bwithout\s+(?:addressing|fixing|changing|extracting|improving\s+extraction)\b)/i;
...
export function gradeEvidenceScope({ reply = '', readWindow = null, evidence = {} } = {}) {
  ...
  if (/\bno\s+(?:(?:linked|separate|recorded)\s+)?(?:tasting|brew|recipe)(?:\s+note)?\s+(?:is\s+)?(?:attached|linked)\s+to\b|\bno\s+(?:tasting|brew|recipe)\s+for\s+that\s+(?:brew|cup)\b|\b(?:v60|kalita|aiden|brew|cup)\b[^?!]{0,128}\b(?:had|has|with)\b[^?!]{0,64}\bno\s+(?:(?:linked|separate|recorded|attached)\s+)?tasting(?:\s+note)?\b/i.test(value)) return [];
