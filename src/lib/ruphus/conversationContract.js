...
export function gradeC5Numbers({ reply = '', userUnits = {} } = {}) {
  ...
  const predictedOutcome = /(?:^|;\s*)\s*(?:[^;.!?]{0,80}\bso\s+)?(?:the\s+(?:finer|coarser)\s+grind|finer|coarser|the\s+change|this(?:\s+change)?|that(?:\s+change)?|(?:the\s+)?(?:(?:muted|dry|flat|sour|thin)\s+)?(?:finish|cup|brew|flavou?r))\s+(?:should|would|could)\s+(?:address|help|increase|decrease|improve|reduce|preserve|keep|add|give|bring|benefit\s+from)\b/i;
