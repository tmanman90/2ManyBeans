...
export function gradeC5Numbers({ reply = '', userUnits = {} } = {}) {
  ...
  const recommendation = /\b(?:try|test|use|make|move|go|adjust|change|increase|decrease|aim|set|turn|start|shift|bump|drop|target|recommend|suggest|should|could|would)\b|\bgrind\s+(?:the\s+)?(?:coffee\s+)?(?:finer|coarser)\b/i;
  const sized = /\b\d+(?:\.\d+)?\s*(?:g|grams?|ml|°?[CF]|steps?|clicks?|notches?|degrees?|seconds?|min(?:ute)?s?)\b|\b(?:ode|grinder)\s+\d+(?:\.\d+)?\b|\b(?:one|two|three|a|an|another)\s+(?:(?:small|tiny|half|gentle|modest)\s+)?(?:(?:finer|coarser)\s+)?(?:grind\s+|dose\s+|ode\s+)?(?:step|gram|click|notch|degree|adjustment|increase|decrease|change)s?(?:\s+(?:finer|coarser))?\b|\b(?:a|an)\s+(?:gentle|modest|small|slight)\s+(?:extraction|strength|temperature|agitation)\s+(?:increase|decrease)\b|\b(?:a\s+)?half\s+(?:a\s+)?(?:step|click|notch|degree)s?\b|\b(?:a\s+(?:little|touch|bit)|slightly)\s+(?:more|less)\s+(?:extraction|agitation|time|heat|water|coffee|dose)\b|\bfrom\s+(?:[A-Za-z]+\s+)?\d+(?:\.\d+)?\s+(?:to|→)\s+\d+(?:\.\d+)?\b/i;
  ...
  const counterfactualAlternative = /(?:^|;\s*)\s*(?:more|less|higher|lower|extra)\s+(?:coffee|dose|water|heat|temperature|time|agitation|extraction)\b[^.!?]*\b(?:could|would|may|might)\b[^.!?]*(?:\bbut\b|\bwhile\s+(?:leaving|keeping|preserving)\b|\band\s+(?:could|would|may|might)\s+(?:leave|keep|preserve)\b|\bwithout\s+(?:addressing|fixing|changing|extracting|improving\s+extraction)\b)/i;
  ...
  const predictedOutcome = /(?:^|;\s*)\s*(?:[^;.!?]{0,80}\bso\s+)?(?:the\s+(?:finer|coarser)\s+grind|finer|coarser|the\s+change|this(?:\s+change)?|that(?:\s+change)?|(?:the\s+)?(?:(?:muted|dry|flat|sour|thin)\s+)?(?:finish|cup|brew|flavou?r))\s+(?:should|would|could)\s+(?:(?:gently|slightly|modestly|directly)\s+)?(?:address|help|increase|decrease|improve|reduce|preserve|keep|add|give|bring|benefit\s+from)\b/i;
