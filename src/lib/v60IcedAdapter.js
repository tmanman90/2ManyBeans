...
import { V60_ICED_SOURCE_REGISTRY_VERSION, V60_ICED_SOURCES, V60_ICED_TECHNIQUES, icedSourceById } from '../data/v60IcedSourceRegistry.js';
...
export function selectV60IcedTechnique(intent = {}, config = {}) {
  ...
  if (text.includes('kurasu') || text.includes('staged iced')) return { ...V60_ICED_TECHNIQUES.kurasuStaged, reasonCode: 'KURASU_STAGED_SOURCE_PROFILE' };
...
function buildRecipe(config, intent, technique, { fallback = false } = {}) {
  ...
  return {
    ...
    ratio: `1:${round(finalBeverageWaterTargetGrams / config.dose * 100) / 100}`, waterTemp: { celsius: clamp(Number(intent?.targetTemperatureC) || 96, 92, 100) }, grindSize: grindFor(config.grinder),
    ...
    steps, postBrewSteps: [{ action: 'After Finish Brew, swirl or stir until the recipe ice has melted as directed; serve over fresh serving ice.', phase: 'post-brew', untimed: true }],
    totalBrewTimeSeconds: guideTargetSeconds, totalBrewTime: timeLabel(guideTargetSeconds), guideTargetSeconds, guideRangeSeconds: [guideTargetSeconds - 30, guideTargetSeconds + 30],
...
export function validateV60IcedCandidate(recipe) {
  ...
  if (!Object.values(V60_ICED_TECHNIQUES).some((technique) => technique.id === recipe.technique)) errors.push('closed-technique-failure');
