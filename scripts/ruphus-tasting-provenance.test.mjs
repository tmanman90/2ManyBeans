...
test('tasting provenance is bound to the immutable attempt snapshot and marks it tasted', () => {
  const attempt = { id: 'attempt-1', coffeeId: 'bean-1', proposalId: 'proposal-1', slotKey: 'v60_hot', revisionId: 'revision-1', snapshotHash: 'hash-1', status: 'completed', snapshot: { method: 'v60' } };
  ...
  assert.equal(result.tasting.agentProvenance.proposalId, 'proposal-1');
  assert.equal(result.tasting.agentProvenance.slotKey, 'v60_hot');
  assert.equal(result.tasting.agentProvenance.coffeeId, 'bean-1');
  assert.equal(result.tasting.agentProvenance.linkedAt, '2026-08-29T12:00:00.000Z');
  ...
  assert.throws(() => applyRuphusTastingState({ attempt: { ...attempt, status: 'created' }, tastingId: 'tasting-3', coffeeId: 'bean-1', sensory: {} }), /completed/);
});
