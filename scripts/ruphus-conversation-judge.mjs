...
export const JUDGE_PROMPT_VERSION = 'ruphus-conversation-judge-prompt-v12';
...
function enforceVisibleProposalRule(result, packet) {
  ...
  const hasProposal = /visible (?:recipe proposal|trial recipe) card:|\b(?:prepared|saved|made|applied)\b[^.!?]{0,48}\b(?:change|proposal|recipe)\b|\b(?:change|proposal|recipe)\b[^.!?]{0,48}\b(?:prepared|saved|made|applied)\b/i.test(assistantText);
