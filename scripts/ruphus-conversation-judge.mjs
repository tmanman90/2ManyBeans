...
function enforceVisibleProposalRule(result, packet) {
  ...
  const hasProposal = /\bproposal\s*:|visible (?:recipe proposal|trial recipe) card:|\b(?:prepared|saved|made|applied|changed)\b[^.!?]{0,48}\b(?:change|proposal|recipe)\b|\b(?:change|proposal|recipe)\b[^.!?]{0,48}\b(?:prepared|saved|made|applied|changed)\b/i.test(assistantText);
  const proposalRequested = /\b(?:go ahead|do it|apply it|apply that|make that change|change it|prepare it|save it|permanent)\b/i.test(userText);
