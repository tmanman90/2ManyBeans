...
assert.match(modal, /timerRecipeForMode\(\{ recipe, attemptId, effectiveDose \}\)/);
...
const resized = timerRecipeForMode({ recipe: { ...recipe, waterGrams: 320, ratio: '1:16' }, effectiveDose: 15 });
assert.equal(resized.coffeeGrams, 15);
assert.equal(resized.waterGrams, 240);
assert.equal(timingContextFromRecipe({ beanId: 'bombe', recipe: resized }).doseGrams, 15);
