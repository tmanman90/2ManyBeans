...
test('Kalita update request produces a persisted card, then explicit Apply changes canonical recipe and Undo restores it', async () => {
  ...
  assert.equal(artifact.sessionId, 'kalita-conversation');
  assert.deepEqual(artifact.actions, ['apply_proposal', 'brew_once', 'keep_current']);
  assert.equal(resolveLegacyRecipe(store.getBean('bean-1'), 'kalita_hot').recipe.waterGrams, 215);
  const preview = createRecipePreview({ recipe: artifact.after, dose: 20 });
  assert.throws(() => repository.createPreview({ uid, coffeeId: 'bean-1', slotKey: 'kalita_hot', sessionId: 'relaunch-context', sourceProposalId: artifact.id, requestId: 'relaunch-wrong-session', after: preview, previewKey: 'relaunch-wrong-session', previewDose: 20, previewRatio: preview.ratio, previewConfiguration: {} }), /bound to this coffee, slot, or session/i);
  const relaunchedPreview = repository.createPreview({ uid, coffeeId: 'bean-1', slotKey: 'kalita_hot', sessionId: artifact.sessionId, sourceProposalId: artifact.id, requestId: 'relaunch-correct-session', after: preview, previewKey: 'relaunch-correct-session', previewDose: 20, previewRatio: preview.ratio, previewConfiguration: {} });
  assert.equal(relaunchedPreview.after.coffeeGrams, 20);
  store.seedProposal(repository.getProposal(uid, artifact.id));
