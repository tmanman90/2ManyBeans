...
export function lockTournamentScores({ packet, scores } = {}) {
  const evaluatorSchedule = PACKET_SCHEDULES.get(packet);
  ...
  return immutableSnapshot({ ...locked, packetHash: packet.packetHash, packetCandidateMapHash: packet.candidateMapHash });
}

export function unblindTournamentScores({ packet, locked } = {}) {
  const evaluatorSchedule = PACKET_SCHEDULES.get(packet);
  if (!object(packet) || packet.type !== 'blind-packet' || !Array.isArray(evaluatorSchedule)) throw new Error('blind packet is required');
