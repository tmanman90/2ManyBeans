...
const trialAction = await receipt.getByRole('button', { name: 'Make this my recipe', exact: true }).boundingBox();
const trialDetails = await receipt.locator('summary').boundingBox();
assert.ok(trialAction.y + trialAction.height <= trialDetails.y, 'Primary save stays above expanded recipe');
assert.ok(trialDetails.height >= 44, 'Recipe disclosure has an iOS-sized tap target');
await receipt.screenshot({ path: '/tmp/ruphus-trial-review-mobile.png' });
await receipt.locator('summary').click();
