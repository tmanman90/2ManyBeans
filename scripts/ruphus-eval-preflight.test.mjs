...
test('identity must be dedicated and capped', () => {
  assert.equal(checkEvaluationIdentity({ projectId: 'eval-1', workspaceId: 'eval-a', quotaUsd: 75 }).ok, true);
  assert.equal(checkEvaluationIdentity({ projectId: 'production', workspaceId: 'eval-a', quotaUsd: 75 }).ok, false);
...
test('preflight requires every telemetry capability', () => {
  assert.equal(validatePreflight({ identity: { projectId: 'e', workspaceId: 'w', quotaUsd: 75 }, env: {}, modelAccess: true, streaming: true, completeUsage: true, requestId: 'r', providerHost: 'https://api.anthropic.com' }).ok, true);
  assert.equal(validatePreflight({ identity: { projectId: 'e', workspaceId: 'w', quotaUsd: 75 }, env: {} }).ok, false);
});
