...
assert.equal(V60_SOURCE_REGISTRY_VERSION, 'v60-hot-sources-v2');
...
assert.equal(V60_SOURCES.find((source) => source.id === 'hoffmann-one-cup-v1').guideSeconds, 180);
assert.equal(V60_SOURCES.find((source) => source.id === 'hoffmann-one-cup-v1').executableCadence, true);
...
assert.deepEqual(V60_SOURCES.find((source) => source.id === 'heart-continuous-v1').pourEndRangeSeconds, [110, 120]);
