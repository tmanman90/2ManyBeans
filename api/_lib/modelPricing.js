...
export const MODEL_PRICING = Object.freeze({
  'gpt-5.6-luna': Object.freeze({ input: 0.40, output: 1.60, cacheRead: 0.04, cacheWrite: 0, source: 'https://developers.openai.com/api/docs/models/gpt-5.6-luna' }),
  'gpt-5.6-terra': Object.freeze({ input: 1.25, output: 10, cacheRead: 0.125, cacheWrite: 0, source: 'https://developers.openai.com/api/docs/models/gpt-5.6-terra' }),
  'gpt-5.4': Object.freeze({ input: 2.50, output: 10, cacheRead: 0, cacheWrite: 0, source: 'https://developers.openai.com/api/docs/models' }),
  'gpt-5.4-mini': Object.freeze({ input: 0.40, output: 1.60, cacheRead: 0, cacheWrite: 0, source: 'https://developers.openai.com/api/docs/models' }),
  'claude-sonnet-5': Object.freeze({ input: 3, output: 15, cacheRead: 0.30, cacheWrite: 3.75, source: 'https://platform.claude.com/docs/en/about-claude/pricing' }),
...
export function normalizeUsage(provider, usage) {
  ...
  let inputTokens; let outputTokens; let cacheReadTokens = 0; let cacheWriteTokens = 0; let reasoningTokens = 0; let thinkingTokens = 0;
  if (provider === 'anthropic') {
    inputTokens = finite(usage.input_tokens); outputTokens = finite(usage.output_tokens);
    cacheReadTokens = finite(usage.cache_read_input_tokens) ?? 0; cacheWriteTokens = finite(usage.cache_creation_input_tokens) ?? 0; thinkingTokens = finite(usage.thinking_tokens) ?? 0;
    ...
    cacheReadTokens = finite(usage.input_token_details?.cached_tokens ?? usage.prompt_tokens_details?.cached_tokens) ?? 0;
  ...
  return Object.freeze({ inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens, reasoningTokens, thinkingTokens, totalTokens: inputTokens + outputTokens });
}
export function calculateCost(model, tokens, registry = MODEL_PRICING) {
  ...
  const value = (tokens.inputTokens * price.input + tokens.outputTokens * price.output + tokens.cacheReadTokens * (price.cacheRead || 0) + tokens.cacheWriteTokens * (price.cacheWrite || 0)) / 1_000_000;
