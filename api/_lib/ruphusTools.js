...
export function createRuphusTools({ uid, context, readers: _readers = {}, proposalStore }) {
  ...
  const call = async (name, args = {}) => {
    ...
    return { ok: true, proposal: clone(proposal), artifact: clone({ ...artifact, ...(proposal?.sourceRevisionId ? { sourceRevisionId: proposal.sourceRevisionId } : {}), ...(proposal?.sessionId ? { sessionId: proposal.sessionId } : {}), ...(proposal?.ownerId ? { ownerId: proposal.ownerId } : {}) }) };
  };
