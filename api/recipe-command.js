...
import { isMutationAllowed, normalizeClientVersion, persistRuphusTrace } from './_lib/ruphusRollout.js';
...
export default withCorsAuth(async (req, res, decodedToken) => {
  ...
  if (MUTATION_MODES.includes(command.mode) && !['timer_started', 'complete_attempt'].includes(command.mode) && !isMutationAllowed({ uid, mode: command.mode, rawUids: process.env.RUPHUS_AGENT_V3_MUTATION_UIDS, rawAccessUids: process.env.RUPHUS_AGENT_V3_UIDS })) return res.status(404).json({ error: 'mutation_unavailable' });
  try {
